...
class MitiApp {
  constructor() {
    ...
    this.isPWA = document.body.classList.contains('pwa-mode');
    this.isExpanded = this.isPWA; // PWA always starts expanded
  ...
  /**
   * Render PWA header card with today's date
   */
  renderPWAHeader() {
    const today = this.converter.getToday();
    const holiday = HolidayManager.getHoliday(today.bs.year, today.bs.month, today.bs.day);
    ...
    if (dayNameEl) dayNameEl.textContent = `${BS_DAYS.en[today.bs.dayOfWeek]} • ${BS_DAYS.np[today.bs.dayOfWeek]}`;
    if (bsDateEl) bsDateEl.textContent = `${today.bs.day} ${BS_MONTHS.en[today.bs.month - 1]} ${today.bs.year}`;
    if (bsDateNpEl) bsDateNpEl.textContent = `${MitiUtils.toNepaliNumeral(today.bs.day)} ${BS_MONTHS.np[today.bs.month - 1]} ${MitiUtils.toNepaliNumeral(today.bs.year)}`;
    if (adDateEl) adDateEl.textContent = MitiUtils.formatADDateShort(new Date());
    if (holidayEl && holiday) {
